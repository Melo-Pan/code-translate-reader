...
function showTranslationHover(
  ...
  // 使用直接的悬浮窗API显示翻译结果
  vscode.languages.registerHoverProvider({ scheme: editor.document.uri.scheme }, {
  ...
  // 方法二：使用传统的装饰方式，但确保正确配置Markdown字符串
  /*
  decorationType = vscode.window.createTextEditorDecorationType({
    after: {
      contentText: '🔍', // 在选中文本后显示放大镜图标
      margin: '0 0 0 5px',
  ...
  */
  ...
  const htmlContent = `
    ...
    <body>
      <div class="container">
        ${showOriginalText ? `<div class="original"><strong>原文:</strong> ${originalText}</div>` : ''}
        <div class="translation"><strong>翻译:</strong> ${translatedText}</div>
      </div>
    </body>
    </html>
  `;
