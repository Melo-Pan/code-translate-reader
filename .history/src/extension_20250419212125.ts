...
async function readTextWithPowerShell(text: string, speed: number): Promise<void> {
  try {
    ...
    // 创建一个PowerShell命令来使用Windows的文本到语音功能
    // 使用System.Speech.Synthesis命名空间中的SpeechSynthesizer类
    const command = `
      Add-Type -AssemblyName System.Speech
      $synth = New-Object System.Speech.Synthesis.SpeechSynthesizer
      $synth.Rate = ${Math.round((speed - 1) * 10)} # 转换速度为Windows的速度范围（-10到10）
      $synth.Speak("${text.replace(/"/g, '`"')}") # 转义双引号
      $synth.Dispose()
    `;
    ...
    throw new Error('Windows朗读服务出错，请确保您有权限执行PowerShell命令');
  }
}
